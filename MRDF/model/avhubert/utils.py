...
def load_video_dfdc(path, image_size, batch_size):
    for i in range(3):
        try:
            ...
            if i == 2:
                print("file exists?",  os.path.exists(path), path)
                # raise ValueError(f"Unable to load {path}")
from decord import VideoReader, cpu
import torch.nn.functional as F
import torch
def load_video(path, scale_percent):
    ...
    for i in range(3):
        try:
            ...
            frames = F.interpolate(frames, size=224, mode='bilinear', align_corners=False)
