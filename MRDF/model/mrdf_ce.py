...
class MRDF_CE(LightningModule):
    ...
    def training_step(self, batch: Optional[Union[Tensor, Sequence[Tensor]]] = None, batch_idx: Optional[int] = None,
        ...
        self.train_outputs.append({"loss": loss_dict["loss"], "preds": preds.detach(), "targets": batch['label'].detach()})
        return {"loss": loss_dict["loss"], "preds": preds.detach(), "targets": batch['label'].detach()}
    ...
    def test_step(self, batch: Optional[Union[Tensor, Sequence[Tensor]]] = None, batch_idx: Optional[int] = None,
        ...
        for i, label in enumerate(batch['label']):
            if 'label' in self.results:
                self.results['label'].append(label.detach())
                self.results['logits'].append(m_logits.detach()[i])
                self.results['video_path'].append(batch['file'][i])
            else:
                self.results['label'] = [label.detach().cpu()]
                self.results['logits'] = [m_logits.detach().cpu()[i]]
                self.results['video_path'] = [batch['file'][i]]
        # from datasets import Dataset
        # results = Dataset.from_dict(self.results)
    ...
    def on_train_epoch_end(self ):
        training_step_outputs = self.train_outputs
    ...
    def on_test_epoch_end(self):
        ...
        from datasets import Dataset
        results = Dataset.from_dict(self.results)
        results.save_to_disk("./predictions_finetuned")
