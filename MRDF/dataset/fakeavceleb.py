...
class MAVOSDDDataset(torch.utils.data.Dataset):

    def __init__(self, subset: str, root: str = "data",
        metadata: Optional[Dataset] = None
    ):
        self.subset = subset
        self.root = root
    ...
    def __getitem__(self, index: int) -> List[Tensor]:
        ...
        # a_label = 1 if 'RealAudio' in meta.type else 0
        # m_label = 1 if 'real' in meta.method else 0
        label = 1 if meta['label']=="real" else 0

        file_path = os.path.join(self.root, meta['video_path'])
        video, audio = self.load_feature(file_path)
        audio, video = torch.from_numpy(
            audio.astype(np.float32)) if audio is not None else None, torch.from_numpy(
            video.astype(np.float32)) if video is not None else None
        ...
        return {"id": index, "file": meta['video_path'], "video": video, 'audio': audio, "label": label}
    ...
    def load_feature(self, mix_name):
        ...
        try:
            ...
            # sample_rate, wav_data = wavfile.read(audio_fn)
            sample_rate, wav_data = video.audio.fps, video.audio.to_soundarray()
            # print(wav_data.shape)
            target_rate = 16_000
            if sample_rate != target_rate:
                # Calculate new number of samples
                num_original_samples = wav_data.shape[0]
                num_target_samples = int(num_original_samples * target_rate / sample_rate)

                # Resample
                if len(wav_data.shape)>1:
                    wav_data = wav_data.mean(axis=1)
                resampled_audio = resample(wav_data, num_target_samples, axis=0)
                wav_data = resampled_audio
            ...
            print("error audio", e)
            audio_feats = np.zeros((video_feats.shape[0], 104))
...
class MAVOSSDataModule(LightningDataModule):
    train_dataset: MAVOSDDDataset
    dev_dataset: MAVOSDDDataset
    test_dataset: MAVOSDDDataset
    metadata: Dataset
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.metadata = load_from_disk(self.root)
...
#############
class PolyGlotFakeDSDataset(torch.utils.data.Dataset):
...
class PolyGlotFakeDataModule(LightningDataModule):
    train_dataset: PolyGlotFakeDSDataset
    dev_dataset: PolyGlotFakeDSDataset
    test_dataset: PolyGlotFakeDSDataset
    metadata: Dataset
    ...
    def __init__(self, root: str = "data",
        ...
        self.take_dev = take_dev
        self.take_test = take_test
        self.Dataset = PolyGlotFakeDSDataset
        self.pad_audio = False
        self.random_crop = False
        self.max_sample_size = 500
        self.train_fold = train_fold
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        for category in ["real", "fake"]:
            for language in os.listdir(os.path.join(self.root, category)):
                lang_path = os.path.join(self.root, category, language)
                
                for movie_name in os.listdir(lang_path):
                    if movie_name.endswith(".mp4"):
                        movie_path = os.path.join(lang_path, movie_name)
                        self.metadata["video_path"].append(movie_path)
                        self.metadata["label"].append(category)
                        self.metadata["split"].append("test")
                    
        test_metadata = pd.DataFrame(self.metadata)
...
class BioDeepAVDSDataset(torch.utils.data.Dataset):
...
class BioDeepAVDataModule(LightningDataModule):
    train_dataset: BioDeepAVDSDataset
    dev_dataset: BioDeepAVDSDataset
    test_dataset: BioDeepAVDSDataset
    metadata: Dataset
    ...
    def __init__(self, root: str = "data",
        ...
        self.take_dev = take_dev
        self.take_test = take_test
        self.Dataset = BioDeepAVDSDataset
        self.pad_audio = False
        self.random_crop = False
        self.max_sample_size = 500
        self.train_fold = train_fold
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        for category in ["real", "fake"]:
            for movie_name in os.listdir(os.path.join(self.root, category, "videos")):
                if movie_name.endswith(".mp4"):
                    movie_path = os.path.join(self.root, category, "videos", movie_name)
                    self.metadata["video_path"].append(movie_path)
                    self.metadata["label"].append(category)
                    self.metadata["split"].append("test")
