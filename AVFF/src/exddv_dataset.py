...
class ExDDV(Dataset):
    def __init__(self, dataset, input_path, audio_conf, stage, num_frames=16):
        ...
        self.dataset["video_path"] = self.dataset.apply(self._obtain_path, axis=1)
        self.dataset["label"] = self.dataset.apply(lambda row: "real" if row["dataset"] == "Original" else "fake", axis=1)
        # if audio_conf.get("mode") == "train":
        #     self.dataset = self.dataset[self.dataset["split"] != "test"]
        # elif audio_conf.get("mode") == "eval":
        #     self.dataset = self.dataset[self.dataset["split"] == "test"]
        # else:
        #     raise Exception(f"split={audio_conf.get('mode')} not implemented.")
        if audio_conf.get("mode") == "train":
            self.dataset = self.dataset[self.dataset["dataset"] != "BioDeepAV"]
        else:
            self.dataset = self.dataset[self.dataset["dataset"] == "BioDeepAV"]
            
            real_biodeepav_path = "/home/eivor/data/BioDeepAV/real/videos"
            real_bideodeepav_videos = [
                os.path.join(real_biodeepav_path,filename) for filename in os.listdir(real_biodeepav_path)[0:100]
            ]
            
            df_temp = pd.DataFrame({
                "movie_name": real_bideodeepav_videos,
                "video_path": real_bideodeepav_videos,
                "label": ["real"] * len(real_bideodeepav_videos),
                "split": ["test"] * len(real_bideodeepav_videos),
                "username": ["Vlad Hondru"] * len(real_bideodeepav_videos),
                "text": ["None"] * len(real_bideodeepav_videos),
                "dataset": ["BioDeepAV"] * len(real_bideodeepav_videos),
                "manipulation": ["original"] * len(real_bideodeepav_videos),
                "click_locations": [{}] * len(real_bideodeepav_videos),
                "difficulty": ["Easy"] * len(real_bideodeepav_videos),
            })
            
            self.dataset = pd.concat((self.dataset,df_temp))
    ...
    def _augment_concat(self, index):
        ...
        sample_1 = self.dataset.loc[index_1]
        video_name_1 = sample_1["video_path"]
...
if __name__ == "__main__":
    audio_conf = {
        'num_mel_bins': 128, 'target_length': 1024, 'freqm': 0, 'timem': 0, 'mode':'test', 
        'mean': -5.081, 'std': 4.4849, 'noise': False, 'label_smooth': 0, 'im_res': 224
    }
    
    csv_path = "/home/eivor/biodeep/xAI_deepfake/dataset5.csv"
    input_path = None
    dataset = ExDDV(csv_path, input_path, audio_conf, stage=2)
    print(dataset[0])
    # print(dataset[150])
    ...
    #     ExDDV(mavos_dd.filter(lambda sample: sample['split']=="train"), "/home/eivor/data/MAVOS-DD", audio_conf, stage=2),
