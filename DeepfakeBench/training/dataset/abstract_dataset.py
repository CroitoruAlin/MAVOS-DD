...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        self.image_list, self.label_list, self.name_list = image_list, label_list, name_list
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                if self.video_level:
                    ...
                    if total_frames >= self.clip_size:
                        ...
                        # video name save
                        path_video = selected_clips[0][0]
                        print(path_video)
                        path_components = path_video.split("/")
                        video_name = os.path.join(path_components[-5], path_components[-4], path_components[-2])
                        video_name_list.extend([video_name] * len(selected_clips))
    ...
    def load_rgb(self, file_path):
        ...
        if not self.lmdb:
            if not file_path[0] == '.':
                file_path =  file_path#f'./{self.config["rgb_dir"]}\\'+file_path
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        if name is not None:
            return image_tensors, label, landmark_tensors, mask_tensors, name
        else:
