...
def video_manipulate(
    ...
    def facecrop(
        ...
        for cnt_frame in frame_idxs:#range(frame_count_org):
            cap_org.set(cv2.CAP_PROP_POS_FRAMES, cnt_frame)
            ...
            if os.path.exists(save_path_) and len(os.listdir(save_path_))>1 and cnt_frame==0:
                    break
...
def preprocess(dataloader, face_detector=None):
    ...
    def img_align_crop(img, landmark=None, outsize=None, scale=1.3, mask=None):
        ...
        # M: use opencv
        # M = cv2.getAffineTransform(src[[0,1,2],:],dst[[0,1,2],:])
        Image.fromarray(img).save("test2.png")
        img = cv2.warpAffine(img, M, (target_size[1], target_size[0]))
        if outsize is not None:
            img = cv2.resize(img, (outsize[1], outsize[0]))
        return img
    for video, path, video_name in tqdm(dataloader):
        ...
        for i, face in enumerate(faces):
            if face is None:
                continue
            face = einops.rearrange(face, "c h w-> h w c").numpy().astype(np.uint8)
            res = 256
            boxes, probs, landmarks = face_detector.detect(face, landmarks=True)
            if landmarks is None:
                continue
            # img = img_align_crop(face, landmarks.squeeze(), outsize= (res,res))
            
            Image.fromarray(face.astype(np.uint8)).resize((res, res)).save(os.path.join(output_dir_frames, f"{i:03}.png"))
            # print(landmarks.shape)
            # exit()
            np.save(os.path.join(output_dir_landmarks, f"{i:03}.npy"), landmarks[0])
            print(output_dir_frames)
...
if __name__ == '__main__':
   ...
   ## DeeperForensics-1.0
    ...
    ## UADFV
        ...
        #     if key not in sub_dataset_paths:
    ...
    preprocess(dataloader, face_detector=face_detector)
