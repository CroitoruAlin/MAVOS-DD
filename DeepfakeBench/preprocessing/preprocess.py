...
def preprocess(movies_path_list, mask_path, mode, num_frames, stride, logger, subdataset_name):
    ...
    if len(movies_path_list) == 0:
        logger.error(f"No videos found")
        sys.exit()
    logger.info(f"{len(movies_path_list)} videos found ")
...
if __name__ == '__main__':
   ...
   ## DeeperForensics-1.0
    ...
    ## UADFV
        ...
        dataset_test_in_domain = dataset.filter(lambda sample: sample['split'] == 'test')
        sub_dataset_paths = {}
    ...
    #             sys.exit()
        ...
        for sub_dataset_name in sub_dataset_paths:
            preprocess(sub_dataset_paths[sub_dataset_name], None, mode, num_frames, stride, logger, sub_dataset_name)
        # for sub_dataset_path in sub_dataset_paths:
